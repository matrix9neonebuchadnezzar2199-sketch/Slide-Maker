...
- 第2弾: `kpi`, `barCompare`, `compare`, `table`
- 第3弾: `process`, `timeline`, `cycle`, `pyramid`, `triangle`
...
追加の AI オプション（いずれもローカル LLM・チェックボックス）:

- **パターン再分類（実験的）**: `content` を `timeline` / `process` / `cycle` / `pyramid` へ変換
- **小見出し（subhead）**: 全角50文字以内の subhead を各スライドに付与
- **スピーカーノート（notes）**: 発表原稿ドラフトを各スライドに付与

### 魔人式 JSON の貼り付け

外部チャット AI（ChatGPT 等）に魔人式 `system prompt_v3.md` を渡して生成した `slideData` JSON を、③の JSON パネルに貼り付けて検証・PPTX 化できます。

- Slide-Maker が描画できる type（14種）はそのまま PPTX 化
- 未対応 type（`faq`, `quote`, `flowChart` 等）は **content に自動降格**（警告表示）
- `notes` はスピーカーノートとして PPTX に書き込み
