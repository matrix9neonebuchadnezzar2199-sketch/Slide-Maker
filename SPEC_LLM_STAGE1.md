...
- プロンプト(システム+ユーザー、日本語、極力単純に):
  - **改善案A**: `content` の `points` 冒頭（最大2件・200文字）も渡し、「このスライド全体の主題を表す見出し」を生成させる。`table` は `headers` と先頭行を要点として渡す。
  - 本文要点が無い場合はタイトルのみの従来プロンプトにフォールバック。

```
system: あなたは日本語の見出しを短くする編集者です。タイトルと本文の要点から、このスライド全体の主題を表す、15文字以内の簡潔な見出しだけを出力してください。説明・記号・カギ括弧・句点は付けないでください。見出し以外は一切出力しないでください。

user: 次のスライド内容にふさわしい、15文字以内の見出しを作ってください。
タイトル: {original_title}
本文の要点:
{slide_context}
見出し:
